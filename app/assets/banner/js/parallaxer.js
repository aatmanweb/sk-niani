...
function aParallax(){
    ...
    this.correctScroll = function (hash, duration){
        ...
        if(p.wndow.scrollTop())
            $('html, body').animate({ scrollTop: 0 }, Math.floor(p.wndow.scrollTop() * 3/2 + 200));
        ...
        $('html, body').animate({ scrollLeft: $(hash).offset().left - p.winHoriSp2Panel }, duration, easing);
...
function mainMenu(){
	$("#mainMenu, #paginationMore, #copyrightlySling .butt").hover(function(){
